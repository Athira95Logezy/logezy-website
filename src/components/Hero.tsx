        ...
        <div style={{ paddingTop: isMobile ? 56 : 96 }}>
          <div style={{ textAlign: 'center', maxWidth: 660, margin: '0 auto', padding: isMobile ? '0 20px 20px' : '0 28px 40px' }}>
            ...
            <motion.h1
              ...
              style={{ fontSize: 'clamp(2.1rem, 5.2vw, 3.85rem)', fontWeight: 900, lineHeight: 1.05, letterSpacing: '-0.048em', color: '#183963', margin: '0 0 18px', textTransform: 'capitalize' as const }}
            >
              The smarter way to{' '}
          ...
          >
            {tripled.map((logo, i) => (
              <div key={`${logo.alt}-${i}`} style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', width: LOGO_W }}>
                <div style={logo.alt === 'Jayco' ? { background: '#114450', borderRadius: 10, padding: '6px 12px', display: 'flex', alignItems: 'center', justifyContent: 'center' } : {}}>
                  <img
                    src={logo.src}
                    alt={logo.alt}
                    draggable={false}
